/* iris_scope — see the mapping
   ============================
   Run this after iris_tilt (GET-STARTED.md, step 8). It makes no sound; it
   makes the network visible.

   You tilt the board and press SPACE, at two or more different poses. Each
   press says "when the board is like this, the numbers should be that." Then
   the network fills in everything in between, and this sketch lets you see
   what it filled in, as a curve, instead of taking it on faith.

   Run the Processing sketch in processing/iris_scope/ and you get:

     the big plot    your demonstrations as dots, and the curve the network
     ...
     the square      the two outputs plotted against each other, the way an
   ...
   You can also read the serial output directly. It is plain text on purpose.
   ...
   WHAT IT SENDS (one thing per line, so you can read it yourself)
     ...
     M text               a message for the person at the computer
   ...
   WHAT IT LISTENS FOR
     ...
     T y0 y1   set that point (Processing sends this when you drag a
               handle, and again before every SPACE)
...
/* ---- what you are teaching it to say ------------------------------------
   The target comes from the Processing window: you drag its two handles to
   where you want this pose to land, and it sends "T y0 y1". No knobs, no extra wiring: an
...
/* The interesting one. This sweeps a pretend input across the whole range
   the sensor has visited and asks the network what it would say at every
   step, including at values you never demonstrated. That sweep is the
   mapping. It costs CURVE_POINTS predictions: about 1.4 ms on the ES3C28P,
   an estimate from board_probe's 15 microseconds for one prediction at a
   slightly larger shape (2 inputs, 12 hidden units, 3 outputs), in the
   library's docs/board/2026-09-25-es3c28p.txt. */
...
/* Are the demonstrations actually far apart? This asks about the poses, not
   ...
   drifting through noise can show a healthy range while every pose was taken
   at the same spot. Teaching two different answers at one input is not
...
static void check_spread(void) {
  ...
  /* One short line, sent last: the plot shows only the latest message, in
     the space beside its own status line. */
  if (hi - lo < 0.5f)
    say("flat curve: your poses are too close. Press c, move further");
}
...
void setup(void) {
...
#if !USE_ANALOG
  /* Find the sensor; do not assume where it is. Every board puts I2C
     somewhere else, and a plain Wire.begin() picks that board's default --
     which on the Adafruit Feather boards is not where the STEMMA QT connector
     is, and on the ES3C28P is not its I2C socket (pins 16 and 15). So: try
     the default, then the pin pairs the common ESP32-S3 boards actually use,
     and try both addresses on each. Whatever answers first wins, and the
     sketch says which so you can hardcode it later if you want to. */
  ...
  /* The sensor's physical full scale, sent once. The plot draws its
     horizontal axis against this and never rescales, so a value keeps its place on the
