...
rows are the walls students actually hit, most common first.
...
(inter-integrated circuit) is the two-wire bus the sensor talks on. **TFT**
(thin-film transistor) is the kind of screen on the board.
...
| The sensor answers at **0x29** instead of 0x28 | The sensor's ADR pad is set | Nothing to do: the sketches try both addresses. |
...
| After holding SAVE and a power cycle, `stemma_bno055` says it loaded the instrument but plays one constant value | A saved instrument with fewer than two demonstrations, or one whose training failed | Tap SAVE at two or more different poses, wait for `trained.`, then hold SAVE until it prints `kept.`. An untrained saved instrument retrains by itself after power-up if it has two or more demonstrations. |
...
| The number or sound follows you, then jumps at one pose | The sketch was changed to read orientation in degrees, which wraps from +180 to −180 | Read the gravity vector, as every sketch here does: it points down and never wraps. |
