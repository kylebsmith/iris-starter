...
as a MIDI device; the other USB Mode, **Hardware CDC and JTAG** (JTAG: Joint
Test Action Group, a debugging interface), is the chip's fixed
serial-and-debug port. **UART** (universal asynchronous
receiver-transmitter) is the chip's plain serial port.
...
| The upload fails, or the port disappears during or after an upload | The board is running a sketch that took over the USB port, or it is stuck between the sketch and its loader | **Hold BOOT, tap RESET, release BOOT**, then upload again. That starts the chip's built-in loader. `iris_tilt` and `iris_instrument` also restart into it when you send a capital **R** in Serial Monitor. The [board log](board-logs/2026-09-25-es3c28p.md) records two routes into the loader working on the ES3C28P with no button: the 1200-baud tap that Upload Mode USB-OTG CDC (TinyUSB) makes, and `iris_tilt`'s **R** in Hardware CDC and JTAG mode. |
...
| A pose you did not mean is in the instrument | It was recorded | For `boilerplate/any_sensor`, type **d** in Serial Monitor and press Enter to delete the last demonstration and retrain, or **c** to clear everything. For `iris_scope`, press **d** or **c** in the plot window. |
...
| The MIDI device appears but the synthesiser does not respond | The synthesiser is not listening to the board, or nothing in it is assigned to controllers 1, 2 and 3 on channel 1 | Choose the board (the Mac's MIDI system names it `ESP32S3_DEV`) as the synthesiser's MIDI input and assign the controllers: [SOUND.md](SOUND.md). |
